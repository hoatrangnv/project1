					...
					<table class="table table-bordered table-hover table-striped dataTable">
						...
						<tbody>
							...
							<tr>
								...
								<td>
									<a href="{{ URL::to('members/refferals/'.$userData->userId.'/detail') }}" class="btn btn-xs btn-info pull-left" style="margin-right: 3px;margin-top: 1px;">{{ trans('adminlte_lang::default.btn_view') }}</a>
								</td>
								<td>
									{{ $userData->loyaltyId }}
        ...
        $(function () {
			...
			$('#submit-command').on('click', function(e) {
				...
				$.ajax({
					...
					$('#modal').modal('hide');
					if (!data.err) {
						if (pushButton) pushButton.prop('disabled', true);
						swal({
							title: "Successfully",
