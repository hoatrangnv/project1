...
class MemberController extends Controller {
    ...
    function getLoyalty($userId) {
        ...
        $loyalty = 0;
        if ($userLoyalty) {
            if ($userLoyalty->isSilver == 1)
                $loyalty = 1;
            if ($userLoyalty->isGold == 1)
                $loyalty = 2;
            if ($userLoyalty->isPear == 1)
                $loyalty = 3;
            if ($userLoyalty->isEmerald == 1)
                $loyalty = 4;
            if ($userLoyalty->isDiamond == 1)
                $loyalty = 5;
        }
        ...
        if ($request->isMethod('post') && Auth::user()->userData->isBinary > 0) {
            if ($request->userSelect > 0 && isset($request['legpos']) && in_array($request['legpos'], array(1, 2))) {
                ...
                if ($userData && $userData->refererId == Auth::user()->id && $userData->isBinary !== 1) {
                    ...
                    User::bonusBinary(
                            $userData->userId, $userData->refererId, $userData->packageId, $userData->binaryUserId, $request['legpos'], false
                    );
